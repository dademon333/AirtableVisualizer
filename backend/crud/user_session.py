...
class UserSessionCRUD:
    ...
    async def get_user_id_by_session_id(session_id: str, redis_cursor: Redis) -> int | None:
        ...
        return await redis_cursor.get(f'session:{session_id}')
    ...
    async def create(user_id: int, redis_cursor: Redis) -> str:
        ...
        await redis_cursor.set(f'session:{session_id}', user_id, ex=3600 * 24 * 30)  # 30 days lifetime
        return session_id

    @staticmethod
    async def delete(session_id: str, redis_cursor: Redis) -> None:
        """Deletes session in redis."""
        await redis_cursor.delete(f'session:{session_id}')
